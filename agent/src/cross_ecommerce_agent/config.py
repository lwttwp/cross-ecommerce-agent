...
# 对外可访问的业务 API 基址(浏览器下载链接用;容器内 BIZ_API_BASE 是 php-web 不可外访)
PUBLIC_API_BASE = os.getenv('PUBLIC_API_BASE', BIZ_API_BASE)
...
8. system 中的 [历史对话摘要] 与 [未处理事项] 是你的内部上下文,
   仅供理解对话背景;除非用户主动询问对应内容,严禁向用户复述或展开朗读它们
"""
...
要求:
1. 只提取消息与上下文中明确出现的信息,绝不编造
2. 消息用指代词指代订单时(如"这个订单/这单/该订单/刚才那单"),
   从【最近对话上下文】中识别对应的订单号填入 order_no
3. 消息与上下文中都没有的信息,一律填 null
4. 金额未提及则 null(表示全额退款)
...
HISTORY_SUMMARY_PROMPT = """你是对话摘要器。把以下历史客服对话概括为一句话会话主题,30 字以内。
只概括主题与用户身份,例如"客服咨询订单查询、任务报表与下载"。
严禁输出任何具体内容: 订单号/任务号/金额/币种/状态/日期/待办事项/诉求细节,
不得出现"用户需要/待/请"等指令性表述。
只输出一句话,不要其他内容。
